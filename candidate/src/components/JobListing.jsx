    ...
    return (
        <div className='container 2xl:px-20 mx-auto flex flex-col lg:flex-row max-lg:space-y-8 py-8 '>
            ...
            <div className="w-full lg:w-1/4  bg-white px-4">
                    ...
                    isSearched && (searchFilter.title !== "" || searchFilter.location !== "") && (
                        <>
                            ...
                            <div className="mb-4 text-gray-600">
                                {searchFilter.title && (
                                    <span className="inline-flex items-center gap-2.5 bg-yellow-600 border border-yellow-900 px-4 py-1.5 rounded ">
                                        {searchFilter.title}
                                        <img onClick={e => setSearchFilter(prev => ({ ...prev, title: "" }))} src={assets.cross_icon} alt="" className="cursor-pointer" />
                                    </span>
                                )}
                                {searchFilter.location && (
                                    <span className="inline-flex items-center gap-2.5 bg-yellow-500  border  border-yellow-800  px-4 py-1.5   ml-2 rounded  ">
                                        {searchFilter.location}
                ...
                <button onClick={e => setShowFilter(prev => !prev)} className="border px-6 py-1.5 rounded border-gray-400 lg:hidden">
                    {
                        showFilter ? "Close"
                            : "Filters"
                    }
                ...
                {filteredJobs.length > 0 && (
                    <div className='flex items-center justify-center space-x-2 mt-10'>
                        <a href="#job-list" className="">
                            <img onClick={() => setCurrentPage(Math.max(1, currentPage - 1))} src={assets.left_arrow_icon} alt="" className="" />
                        </a>
                        {Array.from({ length: Math.ceil(filteredJobs.length / 6) }).map((_, index) => (
                            <a  key={index} href="#job-list" className="">
                                <button className={`w-10 h-10  flex items-center justify-center border border-gray-300 rounded ${currentPage === index + 1 ? 'bg-yellow-200 text-yellow-800' : 'text-gray-500'}`} >{index + 1}</button>
                            </a>
